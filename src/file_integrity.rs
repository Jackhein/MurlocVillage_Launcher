...
mod qobject {
    ...
    impl qobject::FileIntegrity {
        #[qinvokable]
        pub fn check_file(mut self: Pin<&mut Self>) {
            //use qobject::FileIntegrity::{display_message, download_file_process};
            let qt_thread = self.qt_thread();

            thread::spawn(move || {
                //let mut locked_self = thread_this.lock().unwrap();
                //locked_self.as_mut().set_result(QString::from("test"));
                        ...
                        return if let Some(array) = values.as_array() {
                            if array.len() == 2 {
                                ...
                                //         return;
                                    ...
                                    qobject::FileIntegrity::display_message(&qt_thread, &format!("File {} isn't correct, download from {}.", file, url));
                                    return;
                                }
                            } else {
